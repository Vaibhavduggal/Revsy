...
export default async function handler(req, res) {
  ...
  if (req.method === 'POST') {
    ...
    const review = { id: `rev_${Date.now()}_${Math.floor(Math.random() * 1e6)}`, business_id: business.id, customer_id: customerId || null, customer_name: customerName || null, rating: Number.isFinite(Number(rating)) ? Number(rating) : 5, request_id: null, sent_at: null, created_at: new Date().toISOString() };
