  ...
  res.json({
    ...
    recent: (reqs || []).sort((a, b) => new Date(b.created_at) - new Date(a.created_at)).slice(0, 10).map(r => {
      ...
      return {
        ...
        hasCustomMessage: !!(cust && cust.custom_message && cust.custom_message.trim()),
        createdAt: r.created_at,
      };
    }),
    failedSends: (failedSends || []).map(s => ({
      id: s.id,
      customerId: s.customer_id,
      customerName: (customers || []).find(c => c.id === s.customer_id)?.name || s.phone,
      phone: s.phone,
      error: s.error,
