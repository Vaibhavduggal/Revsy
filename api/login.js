...
export default async function handler(req, res) {
  ...
  const valid = bcrypt.compareSync(password, business.password);
  ...
  res.json({
    token,
    business: { id: business.id, name: business.name, ownerEmail: business.owner_email, isAdmin: business.id === 'admin_1' },
  });
}
